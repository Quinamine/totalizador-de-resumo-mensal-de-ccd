...
function preencherCelulasVaziasComZero(){
    ...
    btnAtalhoVazioIgualZero.addEventListener("click", () => {
        ...
        if(celulasVazias > 0) localStorage.setItem("vazio=zero", true);
    ...
    for(const c of celulas) {
        if(localStorage.getItem("vazio=zero") && c.value.length < 1) {
            c.value = 0;
        }
    }
}
...
window.addEventListener("load", () => {
    ...
    btnConfirmarEsvaziarFicha.addEventListener("click", () => localStorage.removeItem("vazio=zero"));
});